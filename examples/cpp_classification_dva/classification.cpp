...
//[deploy.txt 경로] [학습된 모델 파일 경로] [학습된 모델의 mean 파일 경로] [싱크셋 파일 경로] [예측할 이미지 파일 경로]
   ...
   //I:\DME\ClassfyListDir\GATE_ACI I : \DME\ResultOutDir 0
    ...
    if (string(argv[1]) == "CalScore" || string(argv[1]) == "intel")
	{
		...
		while (1)
		{
			...
			for (int i = 0; i < vStrIter.size(); i++)
			{
                ...
                {
                  CValidateModel val(strModelRoot, vStrTrainedFile[i], strImgRoot, strTestListFile, nGpu, true);
                }
                else
                {
		...
		for (int i = 0; i < vClassifyList.size(); i++)
		{
			...
			if (inFile.is_open())
			{
				...
				else
				{
					...
					//full Image;
					if (para[INFO_IDX::WIDTH] == "0" && para[INFO_IDX::HEIGHT] == "0"
						&& para[INFO_IDX::CLIPX] == "0" && para[INFO_IDX::CLIPY] == "0")
					{
						st = clock();
